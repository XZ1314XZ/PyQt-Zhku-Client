...
class zk_spider():
    def __init__(self):
        ...
        if self.mouth > 8 or self.mouth < 3:
            ...
            self.options_seme = '1'
        print(f"当前为{self.year}年{self.mouth}月,{'上' if self.options_seme == '0' else '下'}学期")
        ...
        if os.path.exists('cookies.cok'):
            ...
            with open('cookies.cok', 'r')as f:
                ...
                if fread == '':
                    ...
                    h=requests.get(url='http://jw.zhku.edu.cn/wsxk/stu_xszx_rpt.aspx',
                                            cookies=self.zk_user.home_cookies).text
                    if '无权' in h or '您被强迫下线' in h:
                        print('cookies已失效，开始重新登录')
                    else:
                        self.login_success = 1
                        print('cookies仍然有效')
        else:
            print('未检测到 本地cookies')
        # 主循环
    ...
    def tryLogin(self, user, pw, code):
        # self.zk_user.Get_login_cookies()
        # self.zk_user.Get_code()
        self.zk_user.Get_md5(code)
        with open('cookies.cok', 'w')as f:
            # 开始尝试登录
            self.login_success = self.zk_user.Try_Login_(user, pw)
    ...
    def checkCookiesAndTryLogin(self, user, pw, code, hot=True):
        # 这里目前不完善
        ...
        if os.path.exists('cookies.cok') and hot:
            ...
            with open('cookies.cok', 'r')as f:
                ...
                if fread == '':
                    ...
                    self.tryLogin(user, pw, code)
                else:
                    self.zk_user.home_cookies = eval(fread)
                    if '无权' in requests.get(url='http://jw.zhku.edu.cn/wsxk/stu_xszx_rpt.aspx',
                                            cookies=self.zk_user.home_cookies).text:
                        print('cookies已失效，开始重新登录')
                        self.tryLogin(user, pw, code)
                    else:
    ...
    def Get_SemesterScore(self,year,seme,Sj,SelXNXQ:list,zfx,win):
        """
        :param year 学年
        :param seme 学期
        :param Sj 原始成绩与有效成绩 0,1
        :param SelXNXQ: 入学以来0 学年1 学期 2
        :param zfx: 主辅修
        :return:
        """
        url = 'http://jw.zhku.edu.cn/xscj/Stu_MyScore_rpt.aspx'
        data = {
            "sel_xn": year,
            "sel_xq": seme,
            "SJ": '1' if Sj==True else '0',
            "btn_search": "%BC%EC%CB%F7",
            "SelXNXQ": ['0','1','2'][SelXNXQ.index(True)],
            "zfx_flag":'0' if zfx==True else '1',
            "zxf": '0'
        }
        print(data)
        score_html = requests.post(url=url, cookies=self.zk_user.home_cookies, data=data,timeout=5)
        score_urls = re.findall(
            'Stu_MyScore_Drawimg.aspx\?x=\d{1,}&h=\d{1,}&w=\d{3,}&xnxq=\d{5}&xn=\d{4}&xq=\d&rpt=\d&rad=\d{1}&zfx=\d{1}&xh=\d{12}',
            score_html.text)
        xnxq = {0: '入学以来', 1: '学年', 2: '学期'}[SelXNXQ.index(True)]
        d = f"{year}年第{seme}学期的{xnxq}的{'主修' if zfx == 1 else '辅修'}的{'原始成绩' if Sj == False else '有效成绩'}"
        for i, url in enumerate(score_urls):
            ...
            # t.setSizePolicy(QSizePolicy(QSizePolicy.Preferred,QSizePolicy.Fixed))
            win.ScoreTableWidget.verticalLayout.insertWidget(1,t)
    ...
    def Get_curriculum(self, semester):
        ...
        hidyzm = re.search('[a-z0-9]{31,31}', curriculum_html.text).group(0)
    ...
    def Get_options(self, options_type, options_seme, year):
        ...
        for k, v in course_dict.items():
            print('输入{}--->>选{} 类别:{} 学分:{} 学时:{}'.format(k, course_name[k], course_classification[k], course_socre[k],
                                                          course_time[k]))
        ...
        while 1:
            ...
            for infoi, info in enumerate(course_info_l):
                print(
                    '[{course_info:<{len}}\t'.format(course_info=info + ']',
                                                     len=15 - len(info.encode('GBK', 'ignore')) + len(info)),
                    end='')
    ...
    def withdraw_course(self, options_seme, year):
        ...
        img_url = "http://jw.zhku.edu.cn" + re.search(
            '/znpk/DrawKbimg.aspx\?w=.{,4}&h=.{,4}&xn=.{,4}&xq=.{,1}&zfx=.{,1}&type=.{,5}', txjg.text).group(0)
        ...
        data = {'chkDel5': course_value[eval(tx)],
                ...
                'sel_xnxq': year + options_seme}
        final = requests.post(url='http://jw.zhku.edu.cn/wsxk/stu_txjg_rpt.aspx?func=1',
                              cookies=self.zk_user.home_cookies, data=data)
